...
public class AuthenticationService : IAuthenticationService
{
    ...
    public async Task<UserServerDbModel> CheckSubscriptionExistAsync(Guid ServerId, Guid UserId)
    {
        try
        {
            ...
            if (existingSubscription == null)
            {
                throw new CustomException("User not subscriber of this server", "Check subscription is exist", "User", 401);
            }
    ...
    public async Task<UserServerDbModel> CheckUserNotCreatorAsync(Guid ServerId, Guid UserId)
    {
        try
        {
            ...
            if (existingSubscription == null)
            {
                throw new CustomException("User not subscriber of this server", "Check user is not creator", "User", 401);
            }
            if(await _hitsContext.Server.FirstOrDefaultAsync(s => s.CreatorId == UserId && s.Id == ServerId) != null)
            {
                throw new CustomException("User is creator of this server", "Check user is not creator", "User", 401);
            }
            return existingSubscription;
        }
        catch (CustomException ex)
    ...
    public async Task<UserServerDbModel> CheckUserCreatorAsync(Guid ServerId, Guid UserId)
    {
        try
        {
            ...
            if (existingSubscription == null)
            {
                throw new CustomException("User not subscriber of this server", "Check user is creator", "User", 401);
            }
            if (await _hitsContext.Server.FirstOrDefaultAsync(s => s.CreatorId == UserId && s.Id == ServerId) == null)
            {
                throw new CustomException("User is not creator of this server", "Check user is creator", "User", 401);
            }
            return existingSubscription;
        }
        catch (CustomException ex)
    ...
    public async Task CheckUserRightsSeeChannel(Guid channelId, Guid UserId)
    {
        try
        {
            var channel = await _hitsContext.Channel.Include(c => c.RolesCanView).FirstOrDefaultAsync(c => c.Id == channelId);
            if (channel == null)
            {
                throw new CustomException("Channel not exist", "Check user rights for write in channel", "Channel", 400);
            }
